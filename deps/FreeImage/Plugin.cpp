...
#else
	char *fname = (char *)malloc(wcslen(filename)*2+1);
	unsigned int i=0;
	for(; i < wcslen(filename) * 2; i++) // convert 16-bit to 8-bit
		fname[i] = (char)(filename[i] & 0x00FF);
	// set terminating 0
	fname[i]=0;	
  cout << "C3 " << fname << " " << wcslen(filename) << "\n";
...
if (filename == NULL)
{
  return FIF_UNKNOWN;
	...
	return fRet;
#else
  cout << "A2AA " << (int) filename[0] << "\n";
  cout << "A2A " << wcslen(filename) << "\n";
//   cout << "A2 " << filename << " " << wcslen(filename) << "\n";
  	char *fname = (char *)malloc(wcslen(filename)*2+1);
	unsigned int i=0;
	for(; i < wcslen(filename) * 2; i++) // convert 16-bit to 8-bit
		fname[i] = (char)(filename[i] & 0x00FF);
	// set terminating 0
	fname[i]=0;
  cout << "A3 " << fname << " " << wcslen(filename) << "\n";
	FREE_IMAGE_FORMAT fRet = FreeImage_GetFIFFromFilename(fname);
	free(fname);
