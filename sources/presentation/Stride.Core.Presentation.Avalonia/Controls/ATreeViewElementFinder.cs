...
{
    public static class ATreeViewElementFinder
    {
        [CanBeNull]
        public static ATreeViewItem FindNext(ATreeViewItem treeViewItem, bool visibleOnly)
        {
            while (true)
            {
                // find first child
                ...
                // find next sibling
                var sibling = FindNextSiblingRecursive(treeViewItem) as ATreeViewItem;
                return sibling != null ? (!visibleOnly || sibling.IsVisible ? sibling : null) : null;
            }
        }

        [CanBeNull]
        public static ATreeViewItem GetFirstVirtualizedItem([NotNull] ATreeViewItem treeViewItem)
        {
            for (var i = 0; i < treeViewItem.Items.Count; i++)
            {
                var item = treeViewItem.ItemContainerGenerator.ContainerFromIndex(i) as ATreeViewItem;
                if (item != null)
                    return item;
            }
        ...
        {
            var parentIc = ItemsControl.ItemsControlFromItemContaner(itemsControl);
            if (parentIc == null)
                return null;
        ...
        [CanBeNull]
        public static ATreeViewItem FindFirst([NotNull] ATreeView treeView, bool visibleOnly)
        {
            for (var i = 0; i < treeView.Items.Count; i++)
            {
                var item = treeView.ItemContainerGenerator.ContainerFromIndex(i) as ATreeViewItem;
        ...
        /// <returns>Returns a TreeViewItem.</returns>
        [CanBeNull]
        public static ATreeViewItem FindLast([NotNull] ATreeView treeView, bool visibleOnly)
        {
        ...
        [ItemNotNull]
        public static IEnumerable<ATreeViewItem> FindAll([NotNull] ATreeView treeView, bool visibleOnly)
        {
