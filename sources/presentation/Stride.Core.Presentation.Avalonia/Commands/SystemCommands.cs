...
return true;            
        ...
        private static void ShowSystemMenu([NotNull] Window window)
        {
            ...
            var presenter = global::Avalonia.VisualTree.VisualExtensions.GetVisualChildren(window).Where (x => x is ContentPresenter).Select (x => (ContentPresenter) x).FirstOrDefault(x => Equals(global::Avalonia.VisualTree.VisualExtensions.FindAncestorOfType<Control>((Visual) x), window));
            if (presenter == null)
